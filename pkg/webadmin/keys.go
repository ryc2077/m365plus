...
func openAPIKeys() *apiKeyStore {
	...
	if p == "" {
		if dataDir := strings.TrimSpace(os.Getenv("M365_DATA_DIR")); dataDir != "" {
			p = filepath.Join(dataDir, "api-keys.json")
		} else {
			h, _ := os.UserHomeDir()
...
func (s *apiKeyStore) create(name string) (apiKeyRecord, string, error) {
	return s.createBound(name, "")
}

func (s *apiKeyStore) createBound(name, accountID string) (apiKeyRecord, string, error) {
	b := make([]byte, 32)
	if _, e := rand.Read(b); e != nil {
		return apiKeyRecord{}, "", e
	}
	raw := "m365_" + hex.EncodeToString(b)
	r := apiKeyRecord{ID: hex.EncodeToString(b[:8]), Name: name, Prefix: raw[:12], Hash: keyHash(raw), AccountID: accountID, CreatedAt: time.Now()}
	s.mu.Lock()
...
func (s *apiKeyStore) update(id, name string, revoked *bool) (bool, error) {
	return s.updateFull(id, name, revoked, "")
}

func (s *apiKeyStore) updateFull(id, name string, revoked *bool, accountID string) (bool, error) {
	s.mu.Lock()
	found := false
	for i := range s.Keys {
		if s.Keys[i].ID != id {
...
// boundAccountID returns the account bound to a raw API key. An empty result
// means the key has no explicit binding (round-robin fallback applies).
func (s *apiKeyStore) boundAccountID(raw string) string {
	s.mu.Lock()
	defer s.mu.Unlock()
	h := keyHash(raw)
	for i := range s.Keys {
		if s.Keys[i].Hash == h && !s.Keys[i].Revoked {
			return s.Keys[i].AccountID
		}
	}
	return ""
}
